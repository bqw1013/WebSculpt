...
function parseCategory(input, language) {
  if (!input || typeof input !== "string") {
    const err = new Error("[INVALID_PARAM] category is required");
  ...
  let categoryName = trimmed;

  // Accept full category URLs like https://{lang}.wikipedia.org/wiki/Category:{name}
  if (trimmed.startsWith("http://") || trimmed.startsWith("https://")) {
    let url;
    try {
      url = new URL(trimmed);
    } catch {
      const err = new Error("[INVALID_PARAM] category URL is malformed");
      err.code = "INVALID_PARAM";
      throw err;
    }
    ...
    if (!path.startsWith(prefix)) {
      const err = new Error("[INVALID_PARAM] category URL must point to a Category page");
      err.code = "INVALID_PARAM";
      throw err;
    }
    categoryName = path.slice(prefix.length);
  }

  // Strip optional "Category:" prefix if passed directly
  if (categoryName.startsWith("Category:")) {
    categoryName = categoryName.slice("Category:".length);
  }

  if (!categoryName) {
    const err = new Error("[INVALID_PARAM] category name cannot be empty");
    err.code = "INVALID_PARAM";
    throw err;
  }
...
function parseType(type) {
  const allowed = ["page", "subcat", "file", "all"];
  const value = (type || "page").trim().toLowerCase();
  if (!allowed.includes(value)) {
    const err = new Error(`[INVALID_PARAM] type must be one of ${allowed.join(", ")}`);
    err.code = "INVALID_PARAM";
    throw err;
  }
  return value === "all" ? "page|subcat|file" : value;
}
...
function parseLimit(limit) {
  ...
  if (Number.isNaN(parsed) || parsed < 1 || parsed > 500) {
    const err = new Error("[INVALID_PARAM] limit must be an integer between 1 and 500");
    err.code = "INVALID_PARAM";
    throw err;
  }
  return parsed;
...
function mapType(apiType) {
  if (apiType === "subcat") return "subcat";
  if (apiType === "file") return "file";
  return "page";
}

function buildMemberUrl(language, title) {
  // MediaWiki URL encoding: spaces -> underscores; keep : and / unencoded.
...
export default async function (params) {
  ...
  const categoryName = parseCategory(params.category, language);
  const apiType = parseType(params.type);
  ...
  if (items.length === 0) {
    const err = new Error(`[EMPTY_RESULT] No members found for category "${categoryName}"`);
