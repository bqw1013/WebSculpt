...
npx websculpt command domains

# List only commands under the specified platform
npx websculpt command list <domain>
```
...
npx websculpt <domain> <action> --help

# 2. View full contract card
npx websculpt command show <domain> <action>

# 3. Include README for deep confirmation (use as needed)
npx websculpt command show <domain> <action> --include-readme
```
...
```bash
npx websculpt explore new <name> --intent "<goal description>"
```
...
```bash
npx websculpt explore assess <name>
```
