...
```bash
npx websculpt capture new <name> --domain <domain> --action <action> --runtime <runtime>
```
...
```bash
npx websculpt capture validate <name>
```
...
```bash
npx websculpt capture finalize <name>
```
...
```bash
npx websculpt <domain> <action> --param1 value1
npx websculpt <domain> <action> --param1 value2 --param2 value3
npx websculpt <domain> <action> --param1 ""
npx websculpt <domain> <action> --invalid-param value
```
...
- **Preserve template skeleton**: The export format and function signature in the `command.js` generated by `capture new` are hard requirements of the runtime contract and must be preserved as-is. Only replace `TODO:` comments and placeholder logic (such as `return { ok: true };`); do not modify the export style, function signature, or parameter order.
- **Prioritize reusing explore verification results**: When writing `command.js`, prioritize referencing paths already verified during the explore phase. Verified selectors, API endpoints, interaction steps, and failure signals should be reused in code where possible; if a conflict arises between the verified path and the current implementation, revisit the explore records for completeness rather than discarding the verified path and rewriting from guesswork.
