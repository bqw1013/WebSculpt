...
The command function receives a third argument, `cwd`, which is the current working directory of the CLI process at the time the command is invoked. Because browser runtime executes inside the daemon process, `process.cwd()` inside the command may not match the user's current directory. If the command needs to read from or write to the local file system (for example, to download content or save screenshots), use `path.resolve(cwd, relativePath)` to resolve relative paths to absolute paths.
...
- [ ] Signature is `async (page, params, cwd) => unknown`.
...
- [ ] Preserve the arrow function signature and `page` / `params` / `cwd` parameter order from the generated template.
- [ ] If file I/O is needed, resolve paths with `path.resolve(cwd, ...)`.
