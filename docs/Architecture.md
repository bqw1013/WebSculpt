...
| **B：沉淀工作流** | `capture new / status / validate / finalize` | `.websculpt/captures/<name>/draft/` | Agent 驱动，从零开始沉淀新命令；额外要求 `evidence.md` 和状态机推进 |
| **C：反向导入** | `capture import / status / validate / finalize` | `.websculpt/captures/<name>/draft/` | 修改或维护已有命令；从命令库复制 draft 和 `evidence.md`，合成指纹使工作区初始状态为全部 `done` |

路径 B 和 C 在 finalize 时底层调用与路径 A 相同的安装逻辑，但增加了 evidence 审计和 draft 指纹防篡改等前置门槛。
