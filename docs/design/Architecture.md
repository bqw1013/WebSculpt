...
控制流详见 [Design.md](Design.md)：陌生场景由 AI 探索，探索结果被固化为命令资产，之后直接调用复用；异常时进入自愈闭环。
...
CLI 不只是命令的发现、执行与管理入口，更是 Agent 将过去任务中探索出的经验**固化为可复用代码**，并在后续任务中**约束自身使用这些固化代码**的系统。它是 Agent 的"外置记忆 + 行为约束层"，人类用户和 AI 共享同一套命令库界面。
