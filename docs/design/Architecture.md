...
- 若三者同时存在，`id` 必须等于 `${domain}-${action}`
- `description` 必须存在且为非空字符串
- `runtime` 必须是合法枚举值（`node`、`shell`、`python`、`playwright-cli`）
- `parameters` 如存在必须是对象数组，每个元素为 `{ name, description?, required?, default? }`
- 每个参数必须有 `name`，且 `name` 在数组内唯一
- 参数的 `default` 类型必须是 `string | number | boolean` 之一

> `command create` 会以 CLI 参数为权威，强制覆盖/注入 `id`/`domain`/`action`。`validate` 不带 `[domain] [action]` 时，这三个字段缺失不视为 error，仅发 warning。
...
- **禁止创建或连接浏览器实例**：所有 runtime 的命令代码中均不得出现 `launch`、`connect`、`connectOverCDP`、`newBrowser` 等关键词。`playwright-cli` runtime additionally 不得出现 `chrome-remote-interface`
...
- **node runtime**：代码必须是合法 JavaScript 语法；必须是 ESM 模块格式，包含 `export default` 且导出一个函数
- **playwright-cli runtime**：代码必须是合法 JavaScript 语法；必须包含 `/* PARAMS_INJECT */` 占位符；代码应为函数体形式（非模块），支持具名或匿名函数声明
- **参数一致性（宽松模式）**：只检查代码中以 `params.xxx` 形式访问的参数，对应的 `xxx` 应在 `manifest.parameters` 中声明；未声明的参数发出 `warning`，不阻断落盘
...
```json
{
  ...
  "error": {
    ...
    "details": [
      { "code": "TEMP_REF_FOUND", "message": "...", "level": "error" },
      { "code": "MISSING_PARAMS_INJECT", "message": "...", "level": "error" }
    ]
  }
}
```
...
```json
{
  ...
  "warnings": [
    { "code": "MISSING_README", "message": "...", "level": "warning" }
  ]
}
```
...
`command create` 已有 `--force` 用于覆盖已存在的命令，但**不提供 `--skip-validation`**。无论何种情况，L1-L3 校验失败均阻止落盘。
...
- **身份注入**：落盘前，系统以 CLI 参数 `<domain>` 和 `<action>` 为权威，强制覆盖 manifest 中的 `domain`、`action` 字段，并注入 `id = "${domain}-${action}"`。AI 在草稿阶段无需填写这三个字段
...
- `parameters` 统一为对象数组，不再支持字符串 shorthand（如 `"title"`）
- `context` 统一为 Markdown 字符串，不再支持 `Record<string, unknown>`
...
- `auto`：探索成功后 AI 自行判断是否沉淀
- `proposal`：AI 生成提案，由人类确认后执行
...
沉淀时除 `manifest.json` 和 `command.js` 外，`context.md` 应记录目标网站的结构特征、爬取逻辑依赖的环境条件、可能的失效信号。

异常检测标准、修复触发条件、命令版本策略均未设计。

### 5.5 与 compile 的关系

`command create` 是 compile 校验的唯一落盘入口。它负责 L1-L3 分层校验、保留域保护、身份注入、冲突仲裁、产物完整性检查。`.websculpt-drafts/` 是 AI 的草稿工作区，允许试错；`~/.websculpt/commands/` 是系统的正式档案，只有通过 create 闸机的命令才能进入。
...
**playwright-cli 环境依赖处理**

- `node` runtime 命令可直接测试
- `playwright-cli` runtime 命令需要浏览器环境（CDP 连接）。如果用户未开启远程调试，`command-runner.ts` 返回结构化错误，AI 据此向用户发出明确提示
...
│   ├── compile/             # Reserved for future command specification docs (currently unused)
...
| 命令注册与查找 | `src/cli/engine/registry.ts`（`findCommand`、`listAllCommands`） |
