...
| 命令预检 | `src/cli/meta/validate.ts` | 规划中（CLI 命令未注册） |
| 共享校验逻辑 | `src/cli/meta/command-validation.ts` | 尚未实现（L1-L3 校验未落地） |
...
8. **产物完整性检查**（尚未实现）：README 缺失发 warning；context.md 缺失发 warning
...
3. **实现 L1-L3 校验逻辑**：写入 `src/cli/meta/command-validation.ts`。
4. **实现 `validate` 独立命令**：`src/cli/meta/validate.ts`，复用上述校验逻辑。
