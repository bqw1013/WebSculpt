...
| 命令预检 | `src/cli/meta/validate.ts` | 规划中 |
| 共享校验逻辑 | `src/cli/meta/command-validation.ts` | 尚未实现 |
...
interface CommandManifest {
  ...
  domain: string;
  action: string;
  runtime: CommandRuntime;       // "node" | "shell" | "python" | "playwright-cli"
  parameters?: Array<{
    name: string;
    required?: boolean;
  ...
  outputSchema?: Record<string, unknown>;  // 暂不强制
}
```
...
### 4.6 `command create` 只支持 `--from-dir`
...
```json
{
  ...
  "error": {
    ...
    "message": "Command directory failed validation",
    "details": [
      {
        "code": "TEMP_REF_FOUND",
...
核心职责：
1. **强制校验**：执行 L1-L3，失败阻止落盘
2. **保留域保护**：禁止 `command`、`config` 等保留域
3. **ID 规范化**：校验并补全 id、domain、action 一致性
4. **冲突仲裁**：同名命令无 `--force` 时拒绝覆盖
5. **环境初始化**：自动创建 `~/.websculpt/commands/` 目录树
6. **运行时适配**：根据 `runtime` 确定入口文件名
7. **产物完整性检查**：README 缺失发 warning；context.md 缺失发 warning
8. **审计记录**：记录创建/覆盖事件
...
1. **实现 L1-L3 校验逻辑**：写入 `src/cli/meta/command-validation.ts`。
2. **实现 `validate` 独立命令**：`src/cli/meta/validate.ts`，复用上述校验逻辑。
3. **改造 `command create`**：从 `--from-file` 改为 `--from-dir`，逻辑移入 `src/cli/meta/create.ts`。
4. **拆分 `command.ts`**：将 `create` 移至 `create.ts`，原文件保留 `list/show/remove`。
