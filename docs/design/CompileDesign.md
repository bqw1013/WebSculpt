...
| 命令草稿 | `src/cli/meta/draft.ts` | 已实现（2026-04-24） |
| user/builtin 命令去重 | `src/cli/domains.ts` | 已实现（2026-04-24） |
...
```bash
websculpt command draft <domain> <action> --runtime <rt> [--param ...]
```

CLI 在项目根目录创建：

```
.websculpt-drafts/<domain>-<action>/
  ├── manifest.json      # 预填结构（id/domain/action 留空，create 时注入）
  ├── command.js         # 按 runtime 生成的最小合规骨架（签名 + TODO 注释）
  ├── README.md          # 文档模板
  └── context.md         # 文档模板
```

Agent 编辑业务逻辑后，调用 `command create` 落盘。

`create --from-dir` 仍支持消费任意目录（向后兼容），但 `.websculpt-drafts/` 是 CLI 推荐的草稿工作区。
...
interface CommandManifest {
  ...
  description: string;           // 一句话描述命令用途（必填）
...
- `command draft` 生成的是**最小合规骨架**（仅含正确的入口签名和 TODO 注释），不包含任何业务逻辑预设或填空式代码结构。这不构成"代码模板"，而是将"结构合规"从 Agent 的概率性决策转移为 CLI 的确定性输出。
...
### 4.11 `command draft` 作为草稿生成器

**定位**：`draft` 不是校验器，也不是落盘命令。它生成**已知合规的骨架目录**，让 Agent 从正确的结构起点开始编写业务逻辑。
...
选项：`--runtime`（默认 `node`）、`--to`（默认 `.websculpt-drafts/<domain>-<action>/`）、`--param`（可选，可重复）、`--force`

**`--param` 为可选 convenience**：
- 不提供 `--param` 时，生成空 `parameters` 的 manifest 和纯骨架代码
- 提供 `--param name:required` / `--param name:default=value` 时，CLI 同时在 manifest 和 command.js 中预填参数声明与存取代码，保证跨文件一致性

**输出目录**：`.websculpt-drafts/`（项目根目录下），便于 Agent 的文件工具直接操作

**不运行时校验**：
- draft 的输出是确定性模板拼接，正确性由单元测试保证
- 不调用 `validateCommandPackage` 做自我校验
- Agent 修改后如需预检，显式调用 `command validate`
...
- `description` 必须存在且为非空字符串（不能是空字符串或仅含空白字符）
...
1. **生成合规骨架**：按 runtime 输出正确的代码签名、manifest 结构、文档模板
2. **控制目录结构**：创建 `.websculpt-drafts/` 下的标准化目录
3. **可选参数预填**：通过 `--param` 减少 Agent 的跨文件同步负担
4. **不校验、不落盘、不注入身份**
...
`.websculpt-drafts/` 是 AI 的草稿工作区，允许试错；`~/.websculpt/commands/` 是系统的正式档案，只有通过 create 闸机的命令才能进入。
...
| 命令草稿 | `src/cli/meta/draft.ts`（已实现） |
...
5. ~~**实现 `command draft`**：已完成（2026-04-24）。`src/cli/meta/draft.ts` 已落地，支持 `--runtime`、`--param`、`--to`、`--force`，生成最小合规骨架。~~
6. **修复 `checkJsSyntax` 对 playwright-cli 匿名函数的误报**：已完成（2026-04-24）。`command-validation.ts` 中将 playwright-cli 代码包装为 `return (${code})`，使 `async function (page) { ... }` 形式的匿名函数声明通过语法校验。
7. **修复 user 命令覆盖 builtin 时的重复注册崩溃**：已完成（2026-04-24）。`domains.ts` 中 `registerDomainCommands` 按 action 去重，user 命令优先覆盖 builtin，避免 Commander 抛出 "Command already exists" 错误。
6. **编写规范文档**：`src/compile/README.md`（面向 AI，含 runtime 签名、禁止事项、文档规范模板）。
7. **（可选）启用 outputSchema**：如需输出 schema 校验，需重新设计并添加字段。
...
## 13. 命令编写 Authoring Guidelines

### 13.1 `description` 必填规则

`description` 是 `CommandManifest` 的必填字段，在 L1 结构校验中会被强制检查：

- 必须为 `string` 类型
- 不能是空字符串 `""`
- 不能是仅含空白字符的字符串（如 `"   "`）
- 最佳实践：用一句话清晰描述命令的用途，便于 `command list` 和 help 输出展示

`command draft` 生成的骨架会预填 `"description": ""` 作为占位符，Agent 必须在调用 `command create` 前替换为实际描述。
