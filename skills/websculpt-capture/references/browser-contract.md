...
命令函数接收第三个参数 `cwd`，即调用该命令时 CLI 所在的当前工作目录。由于 browser runtime 在 daemon 进程中执行，命令内的 `process.cwd()` 不一定等于用户当前目录。若命令需要读写本地文件（如下载、保存截图），请使用 `path.resolve(cwd, relativePath)` 将相对路径解析为绝对路径。
...
- [ ] 签名为 `async (page, params, cwd) => unknown`。
- [ ] 不关闭注入的 `page`。
- [ ] 参数转换显式。
...
- [ ] 错误码明确。
- [ ] 实现没有超出 evidence。
- [ ] 保留模板生成的箭头函数签名和 `page` / `params` / `cwd` 参数顺序。
- [ ] 如需文件 I/O，使用 `path.resolve(cwd, ...)` 解析路径。
