...
2. 反复执行 `capture status`，按 artifact 状态推进（填写 evidence 和 draft 文件，执行 `capture validate`），直到所有 artifact 均为 `done`。
3. 执行 `capture finalize` 安装为可执行命令。
4. 执行至少 4 组真实命令测试；不通过则进入修复循环。
...
创建完成后，向用户汇报：同 domain 已有命令列表、是否存在同名冲突、冲突来源（用户命令 / 内置命令）。请求用户确认，拒绝则删除工作区终止；确认后进入状态驱动循环。
...
详细规范见 `./references/node-contract.md` 或 `./references/browser-contract.md`（按 runtime）。
