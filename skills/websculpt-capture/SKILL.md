...
- `userIntentConfirmed` 为 `false` 时，**禁止**执行 `capture new`。必须完成前置确认并获用户明确同意后，方可设为 `true`。
...
1. **前置确认**：向用户展示命令契约（场景、输出、参数、命名、运行时、冲突），获得明确同意后设置 `userIntentConfirmed: true`。
...
确认完成后，将 `CaptureSession.userIntentConfirmed` 设为 `true`，然后执行 `capture new`。
