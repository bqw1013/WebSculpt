...
**探索先决条件**：capture 必须建立在已验证路径之上。如果当前路径尚未经过 explore 阶段验证（无 ExploreSession 记录、无 Capture Assessment），你必须先加载 `websculpt-explore` 完成探索并输出 Capture Assessment，将 `exploreVerified` 设为 `true` 后再进入 capture。
...
- `exploreVerified` 为 `false` 时，**禁止**执行 `capture new`。必须先确认路径已经过 explore 验证；若未验证，必须先加载 `websculpt-explore` 完成探索。
...
- 确认路径已通过 explore 验证后：`exploreVerified` 设为 `true`
