...
- **禁止**在 `guideRead` 为 `true` 之前执行任何 `playwright-cli` 命令或浏览器操作。
...
3. 若判断出需要浏览器自动化，**必须先阅读** `skills/websculpt-explore/references/access/playwright-cli-guide.md`，阅读完成后将 `ExploreSession.guideRead` 设为 `true`，方可执行任何浏览器操作。在此之前严禁执行 `playwright-cli` 任何子命令。
...
- 若 `guideRead` 不为 `true`，先阅读 `skills/websculpt-explore/references/access/playwright-cli-guide.md`
