...
- 若命令返回需要浏览器环境（如提示 `BROWSER_ATTACH_REQUIRED`），立即停止，告知用户需要在 Chrome 中打开 `chrome://inspect/#remote-debugging` 并勾选允许远程调试，保持浏览器打开后重试。同时告知用户：浏览器自动化存在被目标站点检测的风险，可能导致账号风控或访问受限。不要自行尝试启动 daemon 或 attach 浏览器来绕过。只有用户明确说"不用这个命令"或"我自己处理"，才放弃该命令并进入外部工具探索。若用户明确拒绝，后续同一会话中遇到需要浏览器的命令库命令时直接跳过，不再反复询问。
- 若命令库已有命令需要浏览器执行，你不需要阅读 `guide.md`，也不需要操作浏览器。该命令由 **WebSculpt daemon**（独立于 `@playwright/cli` 的后台进程）执行。
...
评估是否产生可沉淀的路径。**判定以路径的结构属性为准，不以用户表述的使用意图为准。** 当路径满足可被唯一标识的目标、可被参数化的输入、以及可预期的输出结构时，即具备沉淀价值。若满足以下任一条件，直接判定为无候选：
- 路径不具备可封装的结构特征，即：访问目标不可被唯一标识，或交互逻辑无法被参数化，或输出结果不可预期
- 只验证了搜索摘要，未读取一手来源
- 路径未实际跑通
- 输出结果不稳定
