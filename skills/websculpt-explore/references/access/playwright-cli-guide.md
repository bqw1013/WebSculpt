...
`@playwright/cli` 是 explore 阶段的浏览器自动化工具，通过 Playwright Extension attach 到用户已打开的 Chrome，复用真实浏览器环境中的登录态、Cookie、LocalStorage 和浏览器指纹。
...
- 无 open 会话 → 按以下步骤建立连接：
  ...
  2. 尝试 Attach 到默认会话：
     ...
     - 若 attach 成功 → 直接进入步骤 3 确认
     - 若报错 `Playwright Extension not found in "..."` → 扩展未安装，引导用户安装：

       ```text
       请访问 https://chromewebstore.google.com/detail/playwright-extension/mmlmfjhmonkocbjadbfplnigmagldckm 安装 Playwright Extension，完成后告诉我继续。
       ```

       用户安装完成后，重新执行本步骤的 attach 命令。
...
首次通过扩展 attach 时，Chrome 会弹出连接授权页面，要求用户点击 Allow。后续可通过 `PLAYWRIGHT_MCP_EXTENSION_TOKEN` 环境变量跳过该弹窗。

attach 前先检查 `PLAYWRIGHT_MCP_EXTENSION_TOKEN` 是否已配置：

- 若已配置，携带 token 执行 attach：
...
- 若未配置，执行无 token 的 attach：

   ```bash
   playwright-cli attach --extension=chrome --session=default
   ```

无 token attach 成功后，connect 页面会显示 `PLAYWRIGHT_MCP_EXTENSION_TOKEN` 值。提示用户复制该值，然后将其配置为当前会话可用的环境变量，并持久化到用户当前使用的 shell 配置中。

后续 attach 会先检查到环境变量已配置，直接携带 token 执行，无需用户再点 Allow。
...
浏览器 session 长期 attach 后，Chrome 内存和扩展桥接开销会持续累积，导致操作响应变慢甚至系统卡顿。以下做法可在**不重新 attach**、**不关闭 session** 的前提下缓解性能衰减。
...
**症状**：执行 `playwright-cli attach --extension=chrome --session=default` 后报错。

**agent 自查与处理**：

1. 运行 `playwright-cli list --all` 查看 Chrome 的扩展状态。
2. 若输出显示 `attach (extension): install at https://chromewebstore.google.com/...`，说明扩展未安装 → 进入「引导用户安装扩展」流程。
3. 若 Chrome 未运行 → 提示用户先启动 Chrome。
4. 若扩展已安装且 Chrome 已运行 → 尝试重新 attach：
...
- 安装完成后，agent 重新执行 attach。

---

### 命令无响应或超时

**症状**：attach 成功、`playwright-cli list` 显示 session open，但 `goto` / `snapshot` / `eval` 等命令超时。
...
- 若上述步骤无效，可能是扩展或浏览器进入异常状态。请用户**重启 Chrome 浏览器**，然后由 agent 重新 attach。

> 扩展模式不依赖 `DevToolsActivePort` 或远程调试开关，因此无需检查端口文件。
