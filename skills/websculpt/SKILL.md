...
> **会话复用提示**：若计划使用浏览器自动化，执行 `playwright-cli list` 检查 `default` 会话是否已 `open`。若已 open，直接复用，无需重新 attach。
...
- **按需复用**：探索过程中，若命令库中存在能覆盖某个子任务的命令，直接调用而非重新实现。通过 `websculpt command list` 查看可用命令，再使用 `websculpt command show <domain> <action>` 查看参数契约与资产完整性
  - 若参数语义或返回值结构在契约卡片中仍不清晰，追加 `--include-readme` 获取 README 中的用法说明、示例和错误码定义
...
- 你使用了 `WebSearch`、`WebFetch`、`curl` 或浏览器自动化等工具获取了信息
- 你访问了特定网站或 API，并成功提取了结构化数据
- 你发现了一条"换参数即可复用"的信息获取路径

**禁止以下行为**：

- 以"这次只是简单搜索"为由跳过沉淀
- 以"用户没要求沉淀"为由跳过沉淀
- 交付答案后直接进入下一轮对话，不评估本次路径

### 查重

提出沉淀前，先执行 `websculpt command list` 检查命令库。若 domain/action 或核心功能已存在，不得重复沉淀；若现有命令可覆盖，标记为"已覆盖"并终止。

### 轻量沉淀询问

发现多个可沉淀路径时**逐一列出**，不要只提一个。示例：

> "本次探索有 2 个可沉淀子任务：1) **hn/list-top**（Firebase API，稳定） 2) **reddit/list-hot**（JSON 接口，需限流）。命令库中均无重叠。是否沉淀？"
...
- 对每个子任务执行查重（见上文）。
- 对通过查重的子任务，判断复用价值并确定 domain/action：
