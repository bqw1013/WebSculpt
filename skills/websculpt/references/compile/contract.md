...
| 章节 | 用途 |
|------|------|
| `## Precipitation Background` | 命令的沉淀历史：为什么创建、解决了什么问题 |
| `## Value Assessment` | 复用价值评估：通用性、复用频率、节省时间，来自沉淀提案卡的 `valueAssessment` |
| `## Page Structure` | 关键 URL、选择器或交互序列 |
| `## Environment Dependencies` | 登录态、浏览器配置、**反爬策略、稳定性说明** |
| `## Failure Signals` | 页面变化时的失效表现（如选择器返回 null、抛出 DRIFT_DETECTED） |
| `## Repair Clues` | 备用方案、替代入口 |

> `## Environment Dependencies` 明确覆盖反爬策略和稳定性说明。沉淀提案卡中的 `antiCrawlAssessment` 和 `stabilityAssessment` 应归纳于此。
...
- [ ] 若 `context.md` 存在，包含 `## Precipitation Background`、`## Value Assessment`、`## Page Structure`、`## Environment Dependencies`、`## Failure Signals`、`## Repair Clues` 章节
