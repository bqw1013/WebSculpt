# WebSculpt Compile 层设计讨论纪要

> 本文件记录截至 2026-04-20 关于 compile 层的设计共识与待决事项。
> 用于后续会话快速恢复上下文，避免重复讨论。

---

## 1. 项目背景

WebSculpt 是一个 CLI 框架，核心命题是：
...
│         │      │              │      │              │
│ cdp/    │      │ 综合策略文档   │      │ 命令编写规范   │
│ playwright/│   │ 工具选择倾向   │      │ 共享校验逻辑   │
└─────────┘      └──────────────┘      └──────────────┘
```

- **access**：确保基础设施就绪（CDP Proxy、Playwright CLI 等），提供各工具的操作参考文档。
...
| access / cdp | `dist/access/cdp/` 有编译产物，`src/access/cdp/` 源码缺失 | 部分落地 |
| access / playwright-cli | `src/access/playwright-cli/README.md` | 仅文档 |
| compile | `src/compile/README.md`（旧版固化指南，已弃用） | 需重写 |
...
- 入口文件：`command.js`
- 标准 ESM 模块，必须 `export default` 一个异步函数
...
playwright-cli 命令执行时，如果用户未开启浏览器远程调试，系统如何识别并返回结构化错误：

- **选项 A（预检）**：`command-runner.ts` 在执行命令前，先调用 `npx playwright-cli list` 探测环境状态。明确可靠，但每次命令多一次 exec 开销（+500ms~2s）。
- **选项 B（后判）**：执行 `run-code` 失败后，根据 stderr/stdout 中的关键词（`attach`、`connect`、`browser`、`Session` 等）启发式判断。零额外开销，但依赖 playwright-cli 错误信息的稳定性，可能误判。
- **选项 C（透传）**：不做特殊处理，让 playwright-cli 原始错误直接透传，由 AI 自行理解。

> 当前 `src/access/playwright-cli/` 仅有一份 README，无代码实现。无论选哪个方案，都需要在该目录补充状态检测或错误转换逻辑。
...
| access / playwright-cli 文档 | `src/access/playwright-cli/README.md` |
| 现有 builtin 命令示例 | `src/cli/builtin/example/hello/`、`src/cli/builtin/zhihu/articles/` |
| compile 旧文档（已弃用） | `src/compile/README.md` |

---

## 9. 下一步建议（供后续会话参考）

1. **先定校验规则清单**：把 7.1~7.4 的选项确定下来。
2. **编写 `src/compile/README.md`**：AI 规范文档，明确各 runtime 的签名和禁止事项。
3. **实现 `src/compile/validator.ts`**：基于确定的规则，实现 `validateCommandPackage()`。
4. **增强 `command create`**：在 `handleCommandCreate` 中集成 validator，替换现有的简单 `validateManifest`。
5. **处理 playwright-cli 错误码**：在 `command-runner.ts` 中增加 `PLAYWRIGHT_CLI_ATTACH_REQUIRED` 的检测与返回。
6. **（可选）L4 outputSchema**：视决策是否启用输出 schema 校验。
