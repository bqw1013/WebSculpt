...
**浏览器自动化示例（browser 运行时）**

以下命令由 WebSculpt daemon 直接连接你的 Chrome/Edge 浏览器执行，**不依赖** `playwright-cli`：
...
> **前置条件**：Chrome/Edge 需要开启远程调试。打开浏览器，访问 `chrome://inspect/#remote-debugging`，勾选"允许此浏览器实例进行远程调试"即可。若命令报错 `BROWSER_ATTACH_REQUIRED`，按上述步骤操作后重试。
...
| **运行时** | `node`（HTTP 请求、数据清洗）或 `browser`（浏览器自动化、复用登录态）。一个命令只能声明一种运行时。 |
