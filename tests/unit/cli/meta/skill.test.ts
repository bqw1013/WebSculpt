	...
	it("removes globally when target exists", () => {
		...
		expect(result).toEqual({
			...
			results: [
				...
				{ agent: "codex", skill: "websculpt-library", status: "not_found" },
	...
	it("returns not_found when target does not exist", () => {
		...
		expect(result).toEqual({
			...
			results: [
				...
				{ agent: "claude", skill: "websculpt-library", status: "not_found" },
